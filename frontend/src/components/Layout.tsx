...
export default function Layout() {
  const navItems = [
    ...
    { to: '/reviews', icon: ClipboardCheck, label: 'Manual Reviews' },
  ];
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col shadow-sm hidden md:flex">
        ...
        <div className="p-4 border-t border-slate-100">
          <button className="flex items-center gap-3 px-3 py-2.5 w-full rounded-lg font-medium text-slate-600 hover:bg-red-50 hover:text-red-600 transition-colors">
            <LogOut size={20} />
            Logout
          </button>
        </div>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-primary-500 to-primary-600 shadow-sm border border-white"></div>
          </div>
        </header>
